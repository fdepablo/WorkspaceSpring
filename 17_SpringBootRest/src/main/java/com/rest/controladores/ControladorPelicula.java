...
public class ControladorPelicula {
	...
	public ResponseEntity<Pelicula> insertar(
		...
		try {
			Pelicula p1 = gp.insertar(pelicula);
			String url = "http://localhost:8080/peliculas/" + p1.getId();
			//return new ResponseEntity<String>(url,HttpStatus.CREATED);//201
			return new ResponseEntity<Pelicula>(p1,HttpStatus.CREATED);//201
		} catch (Exception e) {
			e.printStackTrace();
	...
	//Lo que este metodo produzca va a ir en función de la cabecera accept
	//que le envie el cliente, si es json o xml
	...
	public ResponseEntity<Pelicula> modificarPelicula(
			...
			@PathVariable("id") int id){
		pelicula.setId(id);
		Pelicula p1 = gp.modificar(pelicula);
		return new ResponseEntity<Pelicula>(p1,HttpStatus.OK);		
	}
